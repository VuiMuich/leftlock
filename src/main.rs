...
fn get_pwfile_path() -> String {
    ...
    let file_suffix = String::from("/.leftlock_pwd");
...
// Function to read password from /home/user/.leftlock_pwd if it exists
...
fn getpw(force_create_file: bool) -> String {
    // Read password from /home/user/.leftlock_pwd file
...
fn lockscreen(
    ...
    unsafe {
        ...
        for _ in 0..6 {
            ...
            if ptgrab == GrabSuccess && kbgrab == GrabSuccess {
                ...
                if lockblur {
                    let alpha = 0.5;
                    let opacity = alpha * 0xffffffffu32 as f32;
                    XChangeProperty(
                        dpy,
                        lock.win,
                        XInternAtom(
                            dpy,
                            CString::new("_NET_WM_WINDOW_OPACITY")
                                .unwrap_or_default()
                                .into_raw(),
                            False,
                        ),
                        ...
                        32,
                        PropModeReplace,
                        CString::new(String::from(opacity.to_string()))
                            .unwrap()
    ...
    if ptgrab != GrabSuccess {
        println!(
            "leftlock: unable to grab mouse pointer for screen: {}",
            screen
        );
    }

    if kbgrab != GrabSuccess {
        println!("leftlock: unable to grab keyboard for screen: {}", screen);
    }
...
fn main() {
    ...
    if keyboard_only {
        println!("leftlock is running");
        println!("Enter password to exit");
    }
    let lockblur = args.get_bool("-b");
    ...
    unsafe {
        ...
        for s in 0..nscreens {
            let (lock, success) = lockscreen(dpy, rr, s, colors.clone(), keyboard_only, lockblur);
            match success {
