  ...
  unsafe {
    ...
    for _ in 0..6 {
      ...
      if ptgrab == GrabSuccess && kbgrab == GrabSuccess {
        ...
        let alpha = 0.5;
        let opacity = (alpha * 0xffffff as f32) as u8;
        XChangeProperty(
          dpy,
          lock.win,
          XInternAtom(dpy, CString::new("_NET_WM_WINDOW_OPACITY").unwrap_or_default().into_raw(), False),
          XA_CARDINAL,
          32,
          PropModeReplace,
          // CString::new(opacity.clone()).unwrap().into_raw(),
          CString::new(String::from(opacity.to_string())).unwrap().into_raw() as *const u8,
          1,
        );
        return (lock, true);
      }
